...
namespace Utils {
    template<class T>
    void Print4D(T* tensor, uint4 dim, std::ostream& stream = std::cout) {
            ...
            for (size_t chIdx = 0; chIdx < dim.z; ++chIdx) {
                ...
                for (size_t y = 0; y < dim.y; ++y) {
                    for (size_t x = 0; x < dim.x; ++x) {
                        ...
                        stream << std::scientific << std::setprecision(2) << tensor[idx] << " ";
                    }
                    stream << "\n";
                }
                if (chIdx == 0) break;
    ...
    template<class T>
    void ReadbackAndPrint4D(T* dTensor, uint4 dim, std::ostream& stream = std::cout) {
        ...
        Print4D(readback.data(), dim, stream);
    }
}